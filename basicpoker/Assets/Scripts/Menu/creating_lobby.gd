...
func _on_create_btn_pressed() -> void:
	...
	host.lobby_data.filtered_lobby_name = filter_lobby_name(lobby_name_le.text)
	#host.lobby.lobby_name = Lobby.unfilter_lobby_name(host.lobby.filtered_lobby_name)
...
func filter_lobby_name(lobby_name: String):
	...
	for separation_index: int in separations_in_name:
		filtered_name[separation_index] = LobbyData.name_filter
